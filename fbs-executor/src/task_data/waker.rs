...
fn waker_clone<T: 'static>(s: *const RefCell<TaskData<T>>) -> RawWaker {
...
fn waker_wake<T: 'static>(s: *const RefCell<TaskData<T>>) {
    let waker_rc = unsafe { Rc::from_raw(s) };
    waker_rc.borrow_mut().channel.send(ExecutorCmd::Schedule(waker_rc.clone()));
}

fn waker_wake_by_ref<T: 'static>(s: *const RefCell<TaskData<T>>) {
    let waker_rc = unsafe { Rc::from_raw(s) };
    std::mem::forget(waker_rc.clone());
...
fn waker_drop<T: 'static>(s: *const RefCell<TaskData<T>>) {
